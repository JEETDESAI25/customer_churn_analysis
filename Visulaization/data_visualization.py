...
churn_df = pd.read_csv("Data/churn_data_cleaned.csv")
...
sns.countplot(x="call_failure", hue="complains", data=churn_df, palette="Accent_r")
plt.show()

sns.set(style="white", font_scale=1.5)
sns.countplot(x="charge_amount", hue="ageGroup", data=churn_df, palette="Accent_r")
plt.show()

plt.pie(
    churn_df["ageGroup"].value_counts(),
    labels=churn_df["ageGroup"].value_counts().index.tolist(),
    autopct="%1.0f%%",
    shadow=True,
)
