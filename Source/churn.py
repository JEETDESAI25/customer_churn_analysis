...
churn_df = churn_df.rename(
    columns={
        ...
        "Complains": "complains",
        "Subscription  Length": "subs_len",
        "Charge  Amount": "charge_amount",
        "Seconds of Use": "seconds_of_use",
        "Frequency of use": "freq_of_use",
        "Frequency of SMS": "freq_of_sms",
        "Distinct Called Numbers": "distinct_call_nums",
        "Age Group": "age_group",
...
cols_mustafa["call_failure"] = (
    pd.to_numeric(
        cols_mustafa["call_failure"].astype(str).str.replace(",", ""), errors="coerce"
    )
    .fillna(0)
    .astype(int)
)
...
cols_mustafa["complains"] = (
    pd.to_numeric(cols_mustafa["complains"].astype(str), errors="coerce")
    .fillna(0)
    .astype(int)
)
print(cols_mustafa["complains"].value_counts())
...
cols_mustafa["subs_len"].fillna(value=cols_mustafa["subs_len"].mean(), inplace=True)
...
cols_mustafa["charge_amount"] = (
    pd.to_numeric(
        cols_mustafa["charge_amount"].astype(str).str.replace(",", ""), errors="coerce"
    )
    .fillna(0)
    .astype(int)
)
...
churn_df = churn_df.drop(
    ["call_failure", "complains", "subs_len", "charge_amount"], axis=1
)
...
cols_3["customer_value"] = (
    pd.to_numeric(
        cols_3["customer_value"].astype(str).str.replace(",", ""), errors="coerce"
    )
    .fillna(0)
    .astype(int)
)
